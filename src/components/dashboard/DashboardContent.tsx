...
export function DashboardContent({ currentView, onViewChange }: DashboardContentProps) {
  ...
  // Redirect to home if user navigates to a view they can't access
  useEffect(() => {
    if (currentView !== 'home' && !canAccessView(currentView)) {
      onViewChange('home');
    }
  }, [currentView, canAccessView, onViewChange]);
  ...
  const { data: ingredientsCount = 0 } = useQuery({
    ...
    enabled: canAccessView('ingredients'),
  });
  ...
  const { data: compoundsCount = 0 } = useQuery({
    ...
    enabled: canAccessView('compounds'),
  });
  ...
  const { data: productsCount = 0 } = useQuery({
    ...
    enabled: canAccessView('products'),
  });
  ...
  const renderContent = () => {
    // Guard: if not accessible, render nothing (useEffect handles redirect)
    if (currentView !== 'home' && !canAccessView(currentView)) return null;

    switch (currentView) {
      case 'ingredients':  return <IngredientsPage />;
      case 'compounds':    return <CompoundsPage />;
  ...
  const renderHomeDashboard = () => {
    return (
      <div className="px-6 py-8">
        ...
        {/* Quick Actions — only show actions for accessible views */}
        <TooltipProvider>
        ...
        {/* Main Cards Grid — only show cards for accessible views */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {canAccessView('ingredients') && (
